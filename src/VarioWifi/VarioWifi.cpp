...
void VarioWifi::startTask()
{
    ...
    xTaskCreatePinnedToCore(this->startTaskImpl, "TaskVarioWifi", 5000, this, 2, &_taskVarioWifiHandle, 0);
}
...
bool VarioWifi::connectToWifi(bool withGnuVarioSSID)
{
    if (!withGnuVarioSSID)
    {
        const char *ssid_1;
...
bool VarioWifi::checkDbVersion()
{
    ...
    VARIO_MEMORY_DEBUG_PRINTLN("avant VarioSqlFlight");
    VARIO_MEMORY_DEBUG_PRINTLN(ESP.getFreeHeap());

    VarioSqlFlight varioSqlFlight;

    VARIO_MEMORY_DEBUG_PRINTLN("apres VarioSqlFlight");
    VARIO_MEMORY_DEBUG_PRINTLN(ESP.getFreeHeap());
