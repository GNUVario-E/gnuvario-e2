...
void VarioLanguage::loadConfigurationLangue(const char *filename)
{
	...
	// valeur par defaut, seront surchargées par le fichier le cas échéant
	setDefaultValue();
	...
	isFileLangOK = isFileLangOK && setParameterFromJsonObject(&Message, "UPDATEWWW", TITRE_UPDATEWWW, MAX_CAR_TITRE_UPDATEWWW);
